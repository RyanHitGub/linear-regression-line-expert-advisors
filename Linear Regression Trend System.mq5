//+------------------------------------------------------------------+
//|                               Linear Regression Trend System.mq5 |
//|                                       Copyright 2020, Ryan Smith |
//+------------------------------------------------------------------+
#property copyright "Copyright 2020, Ryan Smith"
#property version   "1.00"
#property description "Linear Regression Trend System"
...
input double maxLinearRegressionSlope = 10;
input int distance_CurrentPrice_ChandelierExit = 15;
input int distance_CurrentPrice_DailyHighLow = 20;
...
input double takeProfit_Pips = 15;
...
//TrackLossesPerDay variables
int counter_Losses = 0;
bool canTrade = true;
ulong dealTicket_Tracking = 0;
...
void OnTick()
{
   ...
   Comment("CE:   " + "\n" + 
           ...
           "LR:   " + "\n" +
           "Slope (" + string(maxLinearRegressionSlope) + "):    " + string(classLR.DetermineSlopeLessThan(maxLinearRegressionSlope)) + 
           "        Slope:    " + string(NormalizeDouble(classLR.getLinearRegressionSlope(), 4)) + "\n\n" 
           
           "Distance:    " + "\n" +
           "Current Price vs Chandelier Exit (" + string(distance_CurrentPrice_ChandelierExit) + "):    " + string(classCE.DetermineDistanceCurrentPriceChandelierExit(distance_CurrentPrice_ChandelierExit, classPA.getCurrentPrice())) + 
           "        Distance:    " + string(NormalizeDouble(classCE.getDistance_CP_CE(), 4)) + "\n\n" +
           
           "Current Price vs Daily High Low (" + string(distance_CurrentPrice_DailyHighLow) + "):    " + string(classCE.DetermineDistanceCurrentPriceDailyHighLow(distance_CurrentPrice_DailyHighLow, classPA.getCurrentPrice())) + 
           "        Distance:    " + string(NormalizeDouble(classCE.getDistance_CP_DailyHighLow(), 4)) + "\n\n" + 
           
           "Daily Losses:    " + string(counter_Losses) + "        Consecutive Losses:    " + string(consecutiveLosses) + "        Lot Size:    " + string(lotSize)
          );
   ...
   if(IsNewCandle_CurrentTimeframe())
   {
      ...
      if(classCE.getMainTrend_Long() && 
         classLR.DetermineSlopeLessThan(maxLinearRegressionSlope) &&
         classCE.DetermineDistanceCurrentPriceChandelierExit(distance_CurrentPrice_ChandelierExit, classPA.getCurrentPrice()) &&
         classCE.DetermineDistanceCurrentPriceDailyHighLow(distance_CurrentPrice_DailyHighLow, classPA.getCurrentPrice()) &&
         !HasOpenPositions() &&
         canTrade)
      {
         //Enter BUY position
...
//+------------------------------------------------------------------+
//Limit trading to a specified amount of losses per day
//+------------------------------------------------------------------+
void TrackLossesPerDay()
{
   //Reset losses on new day (higher timeframe candle)
   if(IsNewCandle_HigherTimeframe())
   {
      counter_Losses = 0;
   }
   ...
   if(dealTicket_Tracking != HistoryDealGetTicket(totalDeals - 1))
   {   
      dealTicket_Tracking = HistoryDealGetTicket(totalDeals - 1);
      
      //Ensure there is a valid ticket
      if(dealTicket_Tracking > 0)
      {
         //Trade closed due to STOP LOSS triggered
         if(HistoryDealGetInteger(dealTicket_Tracking, DEAL_REASON) == DEAL_REASON_SL)
         {
   ...
   //System will trade when daily losses are less than maxLosingTradesPerDay
   if(counter_Losses >= maxLosingTradesPerDay)
   {
      canTrade = false;
   }
   else
